...
# aes(x = model.error, y = comparable.error, size = sale.price, color = comparable.estimate > model.estimate) +
...
p.error.differences <- ggplot(comparables) +
  ...
  aes(fill = comparable.estimate > model.estimate) +
  ...
  annotate('text', label = .annotation.label, x = 3e5 * c(-1,1), y = 50, color = 'black', size = 7, hjust = c(1,0))
